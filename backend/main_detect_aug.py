...
class SubtitleDetect:
    ...
    def text_detector(self):
        '''使用最新的paddle文本检测模型，2025/11'''
        import paddle
        paddle.disable_signal_handler()
        from paddleocr import TextDetection  # 导入新的TextDetection
        # 配置PP-OCRv5模型（可根据需要调整model_name和model_dir）
    ...
    def detect_subtitle(self, img, frame_no=None):
        ...
        # 根据参数保存图片和JSON
        if self.save_img or self.save_json:
